...
def run_module():
    module_args = dict(
        ...
        edge_kind=dict(
            ...
            choices=[
                ...
                'istio',
                'istio-gateway-api',
                'istio-gateway-api-experimental',
            ],
            default='none',
        ),
        ...
        tool_kubectl=dict(
            type='str', required=False, default='/opt/homebrew/bin/kubectl'
        ),
        tool_istioctl=dict(
            type='str', required=False, default='/opt/homebrew/bin/istioctl'
        ),
    )
    result = dict(
        changed=False,
    ...
    tool_kubectl = pathlib.Path(module.params['tool_kubectl']).resolve(strict=True)
    tool_istioctl = pathlib.Path(module.params['tool_istioctl']).resolve(strict=True)
    ...
    if rc == 0 and module.params['state'] == 'absent':
        rc, out, err = module.run_command(
            check_rc=True,
            args=[tool_istioctl, 'uninstall', '-y', '--purge'],
        )
    result['msg'] = err
    result['changed'] = True
    if rc != 0:
