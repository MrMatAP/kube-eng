...
- name: Example invocation of issue_tls_certificate
  ...
  - name: Issue a TLS certificate
    ...
    vars:
      pki_config_path: "{{ host.idp.config_path }}"
      pki_key_filename: idp_key.pem
      pki_csr_filename: idp_cert.csr
      pki_cert_filename: idp_cert.pem
      pki_common_name: "{{ host.idp.name }}.{{ host.dns.zone }}"
      pki_subject_alt_names:
      - "DNS:{{ host.idp.name }}.{{ host.dns.zone }}"
      - "DNS:{{ host.idp.name }}.kind"
      - "DNS:{{ host.idp.name }}"
      - "DNS:localhost"
      pki_key_type: "{{ host.pki.key_type }}"
      pki_key_curve: "{{ host.pki.key_curve }}"
      pki_key_size: "{{ host.pki.key_size }}"
      pki_ca_path: "{{ host.pki.ca_path }}"
      pki_ca_key_path: "{{ host.pki.ca_key_path }}"
      pki_cert_validity: "{{ host.pki.crt_validity }}"
```
