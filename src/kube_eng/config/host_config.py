...
class HostToolCloudProviderMDNSConfig(RootConfigAware):
    ...
    path: pathlib.Path = Field(
        default=pathlib.Path('/opt/homebrew/sbin/cloud-provider-mdns')
    )
...
class HostToolConfig(RootConfigAware):
    ...
    cloud_provider_mdns: HostToolCloudProviderMDNSConfig = Field(
        default_factory=HostToolCloudProviderMDNSConfig
    )
...
class HostPKIConfig(RootConfigAware):
    ...
    extra_ca_path: pathlib.Path | None = Field(
        default=None,
...
class HostDNSConfig(RootConfigAware):
    enabled: bool = Field(
        default=False, description='Whether to enable a local DNS server on this host'
    )
    ...
    host_ip: str = Field(
        default='127.0.0.1',
        description='IP address to expose the DNS server on the host',
    )
    host_port: int = Field(
        default=53, description='Port to expose the DNS server on the host'
    )
    host_control_port: int = Field(
        default=953,
...
class HostPostgresqlConfig(RootConfigAware):
    enabled: bool = Field(
        default=True,
        description='Whether to enable a local PostgreSQL server on this host',
    )
    ...
    host_port: int = Field(
        default=5432, description='Port to expose the PostgreSQL server on the host'
    )


class HostIDPConfig(RootConfigAware):
    enabled: bool = Field(
        default=True, description='Whether to enable a local IDP server on this host'
    )
    ...
    host_ip: str = Field(
        default='127.0.0.1',
        description='IP address to expose the IDP server on the host',
    )
    host_port: int = Field(
        default=8443, description='Port to expose the IDP server on the host'
    )
...
class HostS3Config(RootConfigAware):
    enabled: bool = Field(
        default=True,
        description='Whether to enable a local S3-compatible server on this host',
    )
    name: str = Field(default='s3', description='Name of the S3 server container')
    port: int = Field(
        default=9000, description='Port to run the S3 server on inside the container'
    )
    console_port: int = Field(
        default=9001, description='Port to run the S3 console on inside the container'
    )
    image: str = Field(
        default='rustfs/rustfs:latest', description='S3 server container image'
    )
...
class HostRegistryConfig(RootConfigAware):
    enabled: bool = Field(
        default=True, description='Whether to enable a local OCI registry on this host'
    )
    name: str = Field(
        default='registry', description='Name of the OCI registry container'
    )
    port: int = Field(
        default=5000, description='Port to run the OCI registry on inside the container'
    )
    image: str = Field(
    ...
    host_ip: str = Field(
        default='127.0.0.1',
        description='IP address to expose the OCI registry on the host',
    )
    host_port: int = Field(
        default=5001, description='Port to expose the OCI registry on the host'
    )
...
class HostKafkaConfig(RootConfigAware):
    enabled: bool = Field(
        default=True, description='Whether to enable a local Kafka server on this host'
    )
    name: str = Field(default='kafka', description='Name of the Kafka server container')
    port: int = Field(
        default=9092, description='Port to run the Kafka server on inside the container'
    )
    image: str = Field(
    ...
    host_ip: str = Field(
        default='127.0.0.1',
        description='IP address to expose the Kafka server on the host',
    )
    host_port: int = Field(
        default=9092, description='Port to expose the Kafka server on the host'
    )
